...
#   build/X86/gem5.opt configs/peregrine/peregrine.py --trace --benchmark <bench>
#
...
export GEM5_ROOT GEM5_BIN TRACE_DIR TRACE_OUT_BASE
...
printf "%s\n" "${BENCHMARKS[@]}" | parallel -j "$(nproc)" \
  --env GEM5_ROOT --env GEM5_BIN --env TRACE_DIR --env TRACE_OUT_BASE --env run_bench \
  run_bench
